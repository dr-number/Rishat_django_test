...
ACTION_UNLOCK_APP_OR_WIDGET='14'

ACTION_DEV_BUILD_JS='15'
...
echo -e "$COLOR_TEXT_GREEN[$ACTION_UNLOCK_APP_OR_WIDGET] - Unlock app or widget (need root)"
...
elif [ "$action" == "$ACTION_UNLOCK_APP_OR_WIDGET" ]; then
    echo "Enter name app or widget: "
    read name
    sudo chown -R 1000:1000 "$name"
...
elif [ "$action" == "$ACTION_DEV_BUILD_SEPARATION_CSS" ]; then
    gulp_action='dev-css-separation'
